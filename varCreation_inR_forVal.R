...
all <- read.csv("~/CapOne/Validation_transactions.csv")
head(all)
...
in_zips_shop <- function(id,pairs,merchant_zips){
  merch_z = pairs[pairs$acct_id_num == id,]
  out = rep(0,14)
  for (i in 1:14){
...
at_industry = function(id, transactions, industry){
  out=rep(0,14)
  by_id <- transactions[transactions$acct_id_num == id,]
  for (i in 1:14){
...
################################################################################################################################
s=t=Sys.time()
purchase_online <- aggregate(all$internet_trxn, by=list(all$acct_id_num), 'FUN' = mean)
colnames(purchase_online) <- c("acct_id_code","online_prob")
Sys.time()-s

s=Sys.time()
# aggregate purchases by industry
tmp_industry <- aggregate(rep(1,dim(all)[1]), by=list(all$acct_id_num, as.character(all$Industry_Name)), 'FUN' = sum)
...
freq_count <- aggregate(rep(1,dim(all)[1]), by=list(all$acct_id_num), 'FUN' = sum)
...
freq_count_last3 <- aggregate(rep(1,dim(last3)[1]), by=list(last3$acct_id_num), 'FUN' = sum)
colnames(freq_count_last3) <- c('acct_id_code','freq_prev3')
Sys.time()-s

# Number of Merchants and Merchant ZipCodes
num_zips <- aggregate(rep(1,dim(all)[1]), by=list(all$acct_id_num, as.character(all$merchant_zip)), 'FUN' = sum)
zip_count <- aggregate(rep(1,dim(num_zips)[1]),list(num_zips$Group.1),sum)
colnames(zip_count) =c('acct_id_code','Num_ZIP')

num_merch <- aggregate(rep(1,dim(all)[1]), by=list(all$acct_id_num, as.character(all$merchant_code)), 'FUN' = sum)
...
tmp_industry_online <- aggregate(rep(1,dim(all_online)[1]), by=list(all_online$acct_id_num, as.character(all_online$Industry_Name)), 'FUN' = sum)
...
freq <- aggregate(rep(1,dim(at_merch)[1]),by=list(at_merch$acct_id_num,at_merch$merchant_code),sum)
...
small<-all[!duplicated(paste(all$acct_id_num,all$customer_zip5)),]
a<-strptime(small$trxn_date, '%d%B%Y')
bytime <- small[order(a,decreasing =T),]
customer_zips <- bytime[!duplicated(bytime$acct_id_num),] # retain most recent zip
live_inzip <- sapply(customer_zips$customer_zip5,in_zips, merchant_zips=merchant_zips)
live_inzip <- cbind(customer_zips$acct_id_num,t(live_inzip))
...
pairs <- unique(all[c("acct_id_num", "merchant_zip5")])
customers <- unique(pairs$acct_id_num)
...
industries_only_unique <- unique(industries_only[c("acct_id_num", "Industry_Name")])
s=Sys.time()
...
industries_zip_unique <- unique(industries_only[c("acct_id_num","merchant_zip5", "Industry_Name")])
s=Sys.time()
